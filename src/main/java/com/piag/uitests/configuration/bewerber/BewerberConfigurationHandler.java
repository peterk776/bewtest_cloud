package com.piag.uitests.configuration.bewerber;

import com.google.common.collect.ImmutableList;
import com.google.gson.Gson;
import com.google.gson.JsonElement;
import com.piag.uitests.configuration.AbstractConfigurationHandler;
...
public class BewerberConfigurationHandler extends AbstractConfigurationHandler<BewerberTestConfigurationData> {
    ...
    // Queue used instead of ConcurrentHashmap, want to preserve order how they have been created
    ...
    protected BewerberTestConfigurationData resolveAndAddConfigurationData(Gson gsonBuilder, JsonElement jsonElement) {
        ...
        checkAndRemoveExisting(data);
        if (data != null)
            configurations.add(data);
        return data;
    }

    private void checkAndRemoveExisting(BewerberTestConfigurationData data) {
        boolean removed = configurations.removeIf(exd -> exd.getEndpointUrl().equals(data.getEndpointUrl()) && exd.getCompanyEid().equals(data.getCompanyEid()) && exd.getPositionId().equals(data.getPositionId()));
        if (removed)
            LOGGER.info("Configuration {} will be replaced with new one", data);
    }
}
